...
tests = do
  ...
  describe "className" do
    ...
    it "should reject characters other than lowercase or '-'" do
      let chars = toEnumWithDefaults bottom top <$> 33 .. 126
      let invalidChars = A.filter (\c -> c /= '-' && not (isLower c)) chars
      let xs = S.singleton <$> invalidChars
  ...
  for_ ["child", "last-child", "last-of-type", "of-type"] \x -> do
    ...
    it ("should allow formulas in :nth-" <> x <> "(...) selectors") $
      for_
        ["1", "-1", "n+1", "n-1", "-n+1", "-n-1", "-2n", "-2n+1", "2n", "-2n"]
          \formula ->
    ...
    for_
      (
        let
          ...
          nonLetters =
            A.filter (\c -> not (isLetter c) && c /= ')' && c /= '-') chars
        in do
  ...
  describe "combinator" do
    ...
    it "should reject characters that are not combinators" $
      for_
        (S.toCharArray "!@#$%^&*(){}\\|.,`")
        \x -> runParser (S.singleton x) combinator `shouldSatisfy` isLeft
  ...
  describe "value" do
    ...
    it "should allow quoted and unquoted value segments" $
      ...
      `shouldEqual`
      Right (Value (Simple <$> [ Unquoted [ Lit "red" ]
                               , Quoted [ Lit "yellow" ]
                               , Unquoted [ Lit "green" ]
                               , Quoted [ Lit "blue" ]
                               ]))
  ...
  describe "declaration" do
    ...
    it "should parse a declaration" $
      ...
      `shouldEqual`
      Right (Declaration $ Tuple (Property "font-size")
